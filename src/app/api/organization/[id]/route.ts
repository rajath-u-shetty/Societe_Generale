...
export async function GET(req: Request, { params: { organizationId } }: { params: { organizationId: string } }) {
  const session = await getServerSession(authOptions);
  if (!session) return new Response("Unauthorized", { status: 401 });
  try {
    const isUserInOrganization = await db.organization.findFirst({
      where: {
        id: organizationId,
        users: {
          some: {
    ...
    if (!isUserInOrganization) return new Response("User is not a member", { status: 401 });
    ...
    return new Response("Failed to get organization data", { status: 500 });
  }
}
